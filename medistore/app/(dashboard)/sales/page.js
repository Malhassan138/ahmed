...
export default function SalesPage() {
  ...
  const canDelete = me?.permissions?.includes("*") || me?.permissions?.includes("sales.delete");
  ...
  async function deleteSale() {
    if (!confirm(`تأكيد حذف فاتورة رقم ${detailSale.id} نهائياً من السجل؟ هذا الإجراء لا يمكن التراجع عنه.`)) return;
    const res = await fetch(`/api/sales/${detailSale.id}`, { method: "DELETE" });
    const data = await res.json();
    if (!res.ok) { alert(data.error || "خطأ"); return; }
    setDetailSale(null);
    loadHistory();
            ...
            <div className="flex justify-between items-center border-t pt-3">
              ...
              <div className="flex gap-2">
                {canReturn && !detailSale.returned && (
                  <button className="btn-secondary flex items-center gap-1 text-red-700 border-red-300" onClick={returnWholeSale}>
                    <Undo2 size={14} /> إرجاع كل المتبقي
                  </button>
                )}
                {canDelete && (
                  <button className="btn-danger flex items-center gap-1" onClick={deleteSale}>
