...
import { ShoppingCart, History, Trash2, ReceiptText, Undo2 } from "lucide-react";
...
export default function SalesPage() {
  ...
  function addToCart(p) {
    ...
    if (existing) {
      ...
      setCart([...cart, { productId: p.id, name: p.name, quantity: 1, sellPrice: p.sellPrice, originalPrice: p.sellPrice, maxQty: p.quantity }]);
    }
  }
  ...
  function updatePrice(productId, price) {
    setCart(cart.map((c) => (c.productId === productId ? { ...c, sellPrice: Number(price) } : c)));
  }
  ...
  async function returnSale(sale) {
    if (!confirm(`تأكيد إرجاع فاتورة رقم ${sale.id}؟ هذا سيرجع الكمية للمخزون ولا يمكن التراجع عنه.`)) return;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-[60vh] overflow-y-auto">
            {filtered.map((p) => (
              <button
                ...
                className={`text-right border rounded-lg p-3 hover:border-brand-500 hover:shadow-md hover:-translate-y-0.5 transition-all duration-150 ${p.quantity <= 0 ? "opacity-40 cursor-not-allowed" : ""}`}
              >
        ...
        <div className="card space-y-3">
          ...
          <div className="space-y-2 max-h-[40vh] overflow-y-auto">
            {cart.map((c) => (
              <div key={c.productId} className="flex items-center justify-between text-sm border-b pb-2 gap-1">
                ...
                />
                <button className="text-red-600" onClick={() => removeFromCart(c.productId)}><Trash2 size={16} /></button>
              </div>
            ))}
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="card w-full max-w-3xl space-y-3 max-h-[85vh] overflow-y-auto">
            ...
            <table className="data-table">
              <thead>
                <tr><th>#</th><th>التاريخ</th><th>العميل</th><th>الإجمالي</th><th>بواسطة</th><th>الفاتورة</th><th>مرتجع</th></tr>
              </thead>
              <tbody>
                {history.map((s) => (
                  <tr key={s.id} className={s.returned ? "opacity-50" : ""}>
                    ...
                    <td>
                      {s.returned ? (
                        <span className="text-xs text-red-600 font-semibold">تم الإرجاع</span>
                      ) : canReturn ? (
                        <button className="text-red-600 hover:underline text-xs flex items-center gap-1" onClick={() => returnSale(s)}>
                          <Undo2 size={14} /> إرجاع
                        </button>
                      ) : (
                        "-"
