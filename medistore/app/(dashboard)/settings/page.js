...
function ResetTodayCard() {
  ...
  async function doReset() {
    ...
    setResult({
      ok: true,
      message: `تم التصفير: ${data.sales} فاتورة بيع، ${data.purchases} فاتورة شراء، ${data.movements} حركة مخزون، ${data.salaries || 0} دفعة راتب. تم إرجاع الكميات كما كانت قبل اليوم.`,
    });
    setPassword("");
...
function ResetAllCard() {
  ...
  async function doReset() {
    ...
    setResult({
      ok: true,
      message: `تم التصفير الكامل: ${data.sales} فاتورة بيع، ${data.purchases} فاتورة شراء، ${data.movements} حركة مخزون، ${data.salaries || 0} دفعة راتب، وتصفير كمية ${data.productsReset} صنف. النظام جاهز للبدء ببيانات حقيقية.`,
    });
    setPassword("");
    setConfirmWord("");
    setConfirmStep(false);
