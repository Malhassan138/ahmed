    ...
    <div className="min-h-screen bg-gray-100 py-8 print:bg-white print:py-0">
      <div className="max-w-xl mx-auto bg-white rounded-xl shadow-sm p-8 print:shadow-none print:rounded-none">
        ...
        <div className="text-center mb-6 border-b pb-4">
          <div className="text-xl font-extrabold text-brand-700">أبوغرة استور</div>
          <div className="text-sm text-gray-500">موبايلات واكسسوارات — فاتورة بيع</div>
          {sale.returned && (
            <div className="mt-2 inline-block bg-red-50 text-red-600 text-xs font-bold px-3 py-1 rounded-full">
              تم إرجاع هذه الفاتورة (مرتجع)
            </div>
          )}
        </div>
            ...
            {sale.items.map((it) => (
              <tr key={it.id} className="border-b border-gray-100">
                <td className="py-2">
                  {it.product?.name}
                  {it.returnedQty > 0 && (
                    <div className="text-xs text-red-500">مرتجع: {it.returnedQty}</div>
                  )}
                </td>
                <td className="py-2">{it.quantity}</td>
                <td className="py-2">{it.sellPrice.toFixed(2)}</td>
                <td className="py-2">{((it.quantity - it.returnedQty) * it.sellPrice).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
