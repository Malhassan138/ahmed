...
export async function POST(req) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const salariesDeleted = await tx.salaryPayment.deleteMany({ where: { paidAt: { gte: startOfDay } } });
    const expensesDeleted = await tx.expense.deleteMany({ where: { createdAt: { gte: startOfDay } } });

    return {
      movements: movements.length,
      sales: salesDeleted.count,
      purchases: purchasesDeleted.count,
      salaries: salariesDeleted.count,
