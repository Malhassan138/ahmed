  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {canAdjust && (
          <button className="btn-primary" onClick={() => setShowForm(true)}>+ تعديل جرد</button>
        )}
      </div>
            ...
            <tbody>
              {moves.map((mv) => (
                <tr key={mv.id}>
                  ...
                  <td className={["SALE", "ADJUST_OUT"].includes(mv.type) ? "text-red-600" : "text-green-600"}>
                    {["SALE", "ADJUST_OUT"].includes(mv.type) ? "-" : "+"}{mv.quantity}
                  </td>
                  <td>{mv.reason || "-"}</td>
                  <td>{mv.user?.name}</td>
                </tr>
